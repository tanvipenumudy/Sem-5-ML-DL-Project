...
@app.route('/schonell', methods=['GET', 'POST'])
@login_required
def schonell():
    if(request.method == 'POST'):
        req = request.get_json()
        current_user.schonell = req['score']
        db.session.commit()
        print(current_user.schonell)
    return render_template('schonell.html')

@app.route('/score')
@login_required
def score():
    return render_template('score.html', schonell=current_user.schonell)

@app.route('/wepman', methods=['GET', 'POST'])
@login_required
def wepman():
